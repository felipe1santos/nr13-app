/**
 * Fase 9 · 9F.4.5 — a flag própria de `/livro-registro`.
 *
 * Uma flag POR TELA é o que permite ligar uma e deixar as outras quietas, e o
 * que faz o rollback ser "desligar aquela". Esta é a SEXTA (`busca_v9`,
 * `boot_v9`, `inspecoes_v9`, `prontuarios_v9`, `calibracoes_v9`, `livro_v9`), e
 * o degrau de recuo que ela exige é o que estes testes protegem de verdade.
 *
 ...
 * primeiro erro apagaria as CINCO anteriores de quem as tem ligadas: uma flag
 * derrubando as outras sem ninguém pedir.
 *
...
describe('flag livro_v9', () => {
  ...
  it('organização SEM linha em org_sync não herda a flag', async () => {
    ...
    // ato explícito: nasce desligada, como as outras cinco.
    estado.resposta = { data: null, error: null };
  ...
  it('sai na MESMA consulta das outras — nenhum round-trip novo no boot', async () => {
    ...
    await sincronizarFlagDoServidor();
    expect(estado.colunas[0]).toContain('livro_v9');
    expect(estado.colunas).toHaveLength(1);
  });

  it('BANCO SEM a coluna nova preserva as CINCO anteriores', async () => {
    // O estado entre publicar o bundle e aplicar o SQL.
    estado.colunasAusentes = ['livro_v9'];
    estado.resposta = {
      data: {
        v2_ativa: true,
        busca_v9: true,
    ...
    expect(livroV9Ativa()).toBe(false); // a nova fica desligada — lado barato
    expect(buscaV9Ativa()).toBe(true); // e as cinco que já estavam ligadas CONTINUAM
  ...
  it('sem `livro_v9` NEM `calibracoes_v9` ainda preserva as quatro de baixo', async () => {
    // Recuo de dois degraus: o banco anterior à 9F.3.
    estado.colunasAusentes = ['livro_v9', 'calibracoes_v9'];
    estado.resposta = {
      data: {
        v2_ativa: true,
        busca_v9: true,
  ...
  it('A ESCADA INTEIRA: banco anterior a toda a 9F ainda preserva busca_v9 e boot_v9', async () => {
    // Cinco degraus de queda de uma vez — o estado da produção quando a 9E
    // fechou.
    estado.colunasAusentes = ['livro_v9', 'calibracoes_v9', 'prontuarios_v9', 'inspecoes_v9'];
