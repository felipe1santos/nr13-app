...
function larguraBocalPx(b: BocalModelo, scale: number): number {
  const diam = num(b.diametro);
  return clamp((diam === null ? 60 : diam) * scale, 5, 18);
}
...
function svgLongitudinalHorizontal(m: ModeloVaso, d: DadosBase): string {
  ...
  const RESERVA_ESQ = 64; // cota Ø + bocais do tampo1
  const RESERVA_DIR = 52; // bocais do tampo2 + cota do suporte
  const RESERVA_TOPO = 52; // stubs de cima + callout t
  const temSuporte = m.suporte.tipo !== 'nenhum';
  const reservaBaixo = (temSuporte ? 30 : 0) + 34 + 14 + nAx * 13 + 12;
  ...
  parts.push(`<rect x="${xCascoEsq.toFixed(1)}" y="${yTop.toFixed(1)}" width="${(xCascoDir - xCascoEsq).toFixed(1)}" height="${Dpx.toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  const mapEsq: MapaLocal = (z, r) => [xCascoEsq - z * scale, yCentro + r * scale];
  const mapDir: MapaLocal = (z, r) => [xCascoDir + z * scale, yCentro + r * scale];
  parts.push(`<path d="${pathTampoOutline(m.tampo1.tipo, d.DExt / 2, d.prof1, mapEsq)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  parts.push(`<path d="${pathTampoOutline(m.tampo2.tipo, d.DExt / 2, d.prof2, mapDir)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);

  // Costuras: linhas de tangência (junção tampo-casco) + divisões de virolas
  costura(parts, xCascoEsq, yTop, xCascoEsq, yBottom);
  costura(parts, xCascoDir, yTop, xCascoDir, yBottom);
  for (let i = 1; i < d.virolas; i++) {
    const x = clamp(xCascoEsq + ((xCascoDir - xCascoEsq) * i) / d.virolas, MARGIN, VB_W - MARGIN);
    costura(parts, x, yTop, x, yBottom);
  }

  // Linha de centro
  const clX1 = clamp(xEsq - 12, MARGIN, VB_W - MARGIN);
  const clX2 = clamp(xDir + 12, MARGIN, VB_W - MARGIN);
  parts.push(`<line x1="${clX1.toFixed(1)}" y1="${yCentro.toFixed(1)}" x2="${clX2.toFixed(1)}" y2="${yCentro.toFixed(1)}" stroke="${STROKE_CL}" stroke-width="0.7" stroke-dasharray="${DASH_CL}"/>`);
  parts.push(`<text x="${clamp(clX2 + 3, MARGIN, VB_W - MARGIN - 8).toFixed(1)}" y="${(yCentro + 2.5).toFixed(1)}" font-size="8" fill="${STROKE_CL}">CL</text>`);
  ...
  if (temSuporte) {
    if (m.suporte.tipo === 'selas') {
      ...
      for (const frac of [0.2, 0.8]) {
        ...
        parts.push(`<polygon points="${p1.toFixed(1)},${(yBottom - 2).toFixed(1)} ${p2.toFixed(1)},${(yBottom - 2).toFixed(1)} ${p3.toFixed(1)},${(yBase - 3).toFixed(1)} ${p4.toFixed(1)},${(yBase - 3).toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
        parts.push(`<rect x="${(p4 - 2).toFixed(1)}" y="${(yBase - 3).toFixed(1)}" width="${(p3 - p4 + 4).toFixed(1)}" height="3" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
      }
      ...
      parts.push(`<rect x="${xSaia.toFixed(1)}" y="${yBottom.toFixed(1)}" width="${wSaia.toFixed(1)}" height="${(yBase - yBottom).toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
    } else {
      ...
      for (const frac of [0.2, 0.8]) {
        ...
        parts.push(`<rect x="${(xc - legW / 2).toFixed(1)}" y="${(yBottom - 1).toFixed(1)}" width="${legW.toFixed(1)}" height="${(yBase - yBottom - 2).toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
        parts.push(`<rect x="${(xc - legW / 2 - 5).toFixed(1)}" y="${(yBase - 3).toFixed(1)}" width="${(legW + 10).toFixed(1)}" height="3" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
      }
    }
    // Cota da altura do suporte (se informada)
    if (alturaSup !== null) {
      const xCotaSup = clamp(xDir + 22, MARGIN, VB_W - MARGIN);
      cota(parts, xCotaSup, yBottom, xCotaSup, yBase, fmt(alturaSup, 0), VB_W);
    }
  }
  ...
  for (const b of m.bocais) {
    ...
    const wPx = larguraBocalPx(b, scale);
    ...
    if (b.local === 'tampo1' || b.local === 'tampo2') {
      ...
      const tip = bocalFlangeado(parts, clamp(xBase, MARGIN, VB_W - MARGIN), yBocal, isTampo1 ? Math.PI : 0, 18, wPx);
      const bx = clamp(tip.x + (isTampo1 ? -12 : 12), MARGIN + 10, VB_W - MARGIN - 10);
      idBalao(parts, bx, clamp(yBocal, MARGIN + 10, VB_H - MARGIN - 10), esc(b.id));
      continue;
    }
    ...
    if (setor === 'a0' || setor === 'a180') {
      ...
      const tip = bocalFlangeado(parts, xBocal, yBaseBocal, emCima ? -Math.PI / 2 : Math.PI / 2, 20, wPx);
      const by = clamp(tip.y + (emCima ? -12 : 12), MARGIN + 10, VB_H - MARGIN - 10);
      idBalao(parts, clamp(xBocal, MARGIN + 10, VB_W - MARGIN - 10), by, esc(b.id));
    } else {
      ...
      const rBocal = clamp(((diamB === null ? 60 : diamB) * scale) / 2, 3.5, RPx * 0.72);
      ...
      const bx = clamp(xBocal + rBocal + 14, MARGIN + 10, VB_W - MARGIN - 10);
      const by = clamp(yProj - rBocal - 6, MARGIN + 10, VB_H - MARGIN - 10);
      parts.push(`<line x1="${(xBocal + rBocal * 0.7).toFixed(1)}" y1="${(yProj - rBocal * 0.7).toFixed(1)}" x2="${(bx - 8).toFixed(1)}" y2="${by.toFixed(1)}" stroke="${STROKE}" stroke-width="0.6"/>`);
      idBalao(parts, bx, by, esc(b.id));
    }
  }
  ...
  for (const b of bocaisCasco) {
    ...
    const cotaY = clamp(yBase + 12 + idxCotaAxial * 13, MARGIN, VB_H - MARGIN);
  ...
  const yCotaCil = clamp(yBase + 12 + nAx * 13 + 6, MARGIN, VB_H - MARGIN);
  cota(parts, xCascoEsq, yCotaCil, xCascoDir, yCotaCil, `${fmt(d.L, 0)}`, VB_W);
  const yCotaTotal = clamp(yCotaCil + 14, MARGIN, VB_H - MARGIN);
  cota(parts, xEsq, yCotaTotal, xDir, yCotaTotal, `Total: ${fmt(d.comprTotal, 0)}`, VB_W);

  const xCotaD = clamp(xEsq - 28, MARGIN, VB_W - MARGIN);
  ...
  callout(parts, xAlvoT, yTop, clamp(xAlvoT + 26, MARGIN, VB_W - MARGIN - 40), clamp(yTop - 26, MARGIN + 8, VB_H - MARGIN), `t=${fmt(d.tCasco, 1)}`);
...
function svgLongitudinalVertical(m: ModeloVaso, d: DadosBase): string {
  ...
  const RESERVA_ESQ = 56 + nAx * 14; // cotas axiais escalonadas à esquerda
  const RESERVA_DIR = 68; // cotas L/total + stubs à direita
  const RESERVA_TOPO = 56; // bocal do tampo1 + balão
  const temSuporte = m.suporte.tipo !== 'nenhum';
  const RESERVA_BAIXO = (temSuporte ? 38 : 0) + 34 + 30; // suporte + cota Ø + bocal tampo2
  ...
  // Costado + tampos
  parts.push(`<rect x="${xEsq.toFixed(1)}" y="${yCascoTop.toFixed(1)}" width="${(xDir - xEsq).toFixed(1)}" height="${(yCascoBottom - yCascoTop).toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  ...
  parts.push(`<path d="${pathTampoOutline(m.tampo1.tipo, d.DExt / 2, d.prof1, mapTopo)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  parts.push(`<path d="${pathTampoOutline(m.tampo2.tipo, d.DExt / 2, d.prof2, mapBaixo)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);

  // Costuras: tangências + virolas
  costura(parts, xEsq, yCascoTop, xDir, yCascoTop);
  costura(parts, xEsq, yCascoBottom, xDir, yCascoBottom);
  for (let i = 1; i < d.virolas; i++) {
    const y = clamp(yCascoTop + ((yCascoBottom - yCascoTop) * i) / d.virolas, MARGIN, VB_H - MARGIN);
    costura(parts, xEsq, y, xDir, y);
  }

  // Linha de centro vertical
  const clY1 = clamp(yTop - 12, MARGIN, VB_H - MARGIN);
  const clY2 = clamp(yBottom + 12, MARGIN, VB_H - MARGIN);
  parts.push(`<line x1="${centerX.toFixed(1)}" y1="${clY1.toFixed(1)}" x2="${centerX.toFixed(1)}" y2="${clY2.toFixed(1)}" stroke="${STROKE_CL}" stroke-width="0.7" stroke-dasharray="${DASH_CL}"/>`);
  parts.push(`<text x="${clamp(centerX + 4, MARGIN, VB_W - MARGIN - 10).toFixed(1)}" y="${clamp(clY1 - 3, MARGIN + 8, VB_H - MARGIN).toFixed(1)}" font-size="8" fill="${STROKE_CL}">CL</text>`);
  ...
  if (temSuporte) {
    if (m.suporte.tipo === 'saia') {
      ...
      parts.push(`<polygon points="${p.map(([x, y]) => `${x.toFixed(1)},${y.toFixed(1)}`).join(' ')}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
      parts.push(`<rect x="${(p[3][0] - 4).toFixed(1)}" y="${(yBase - 3).toFixed(1)}" width="${(p[2][0] - p[3][0] + 8).toFixed(1)}" height="3" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1"/>`);
    } else {
  ...
  for (const b of m.bocais) {
    ...
    if (b.local === 'tampo1' || b.local === 'tampo2') {
      ...
      const tip = bocalFlangeado(parts, clamp(xBocal, MARGIN, VB_W - MARGIN), clamp(yBaseBocal, MARGIN, VB_H - MARGIN), isTampo1 ? -Math.PI / 2 : Math.PI / 2, 18, wPx);
      const by = clamp(tip.y + (isTampo1 ? -12 : 12), MARGIN + 10, VB_H - MARGIN - 10);
      idBalao(parts, clamp(xBocal, MARGIN + 10, VB_W - MARGIN - 10), by, esc(b.id));
      continue;
    }
    ...
    if (setor === 'a90' || setor === 'a270') {
      ...
      const tip = bocalFlangeado(parts, direita ? xDir : xEsq, yBocal, direita ? 0 : Math.PI, 18, wPx);
      const bx = clamp(tip.x + (direita ? 12 : -12), MARGIN + 10, VB_W - MARGIN - 10);
      idBalao(parts, bx, clamp(yBocal, MARGIN + 10, VB_H - MARGIN - 10), esc(b.id));
    } else {
      ...
      const bx = clamp(xProj + rBocal + 14, MARGIN + 10, VB_W - MARGIN - 10);
      const by = clamp(yBocal - rBocal - 8, MARGIN + 10, VB_H - MARGIN - 10);
      parts.push(`<line x1="${(xProj + rBocal * 0.7).toFixed(1)}" y1="${(yBocal - rBocal * 0.7).toFixed(1)}" x2="${(bx - 8).toFixed(1)}" y2="${by.toFixed(1)}" stroke="${STROKE}" stroke-width="0.6"/>`);
      idBalao(parts, bx, by, esc(b.id));
    }
  }
  ...
  for (const b of bocaisCasco) {
    ...
    const cotaX = clamp(xEsq - 42 - idxCotaAxialV * 14, MARGIN, VB_W - MARGIN);
    cota(parts, cotaX, yCascoTop, cotaX, yBocal, `${esc(b.id)}: ${fmt(posAxial, 0)}`, VB_W);
    idxCotaAxialV++;
  }

  // Cotas principais: Ø (embaixo do suporte), L e total (à direita), espessura
  const yCotaD = clamp(yBase + 22, MARGIN, VB_H - MARGIN);
  cota(parts, xEsq, yCotaD, xDir, yCotaD, `Ø${fmt(d.D, 0)}`, VB_W);

  const xCotaCil = clamp(xDir + 24, MARGIN, VB_W - MARGIN);
  cota(parts, xCotaCil, yCascoTop, xCotaCil, yCascoBottom, `${fmt(d.L, 0)}`, VB_W);
  const xCotaTotal = clamp(xDir + 44, MARGIN, VB_W - MARGIN);
  ...
  callout(parts, xDir, yAlvoT, clamp(xDir + 26, MARGIN, VB_W - MARGIN - 36), clamp(yAlvoT - 18, MARGIN + 8, VB_H - MARGIN), `t=${fmt(d.tCasco, 1)}`);
...
function svgTransversal(m: ModeloVaso, d: DadosBase): string {
  ...
  // Reserva externa: stub flangeado (20) + balão (12) e/ou arcos escalonados (13 cada)
  const reservaExterna = Math.max(36, 24 + Math.min(nArcos, 5) * 13);
  ...
  const tPx = clamp(d.tCasco * scale, 2, 14);
  const rInt = r - tPx;

  const parts: string[] = [];

  // Anel de parede (corte): círculo externo + interno
  parts.push(`<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  parts.push(`<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${rInt.toFixed(1)}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="0.9"/>`);

  // Cruz de centro tracejada
  const crossExt = r + 8;
  parts.push(`<line x1="${clamp(cx - crossExt, 2, VB_W - 2).toFixed(1)}" y1="${cy.toFixed(1)}" x2="${clamp(cx + crossExt, 2, VB_W - 2).toFixed(1)}" y2="${cy.toFixed(1)}" stroke="${STROKE_CL}" stroke-width="0.7" stroke-dasharray="${DASH_CL}"/>`);
  parts.push(`<line x1="${cx.toFixed(1)}" y1="${clamp(cy - crossExt, 2, VB_H - 2).toFixed(1)}" x2="${cx.toFixed(1)}" y2="${clamp(cy + crossExt, 2, VB_H - 2).toFixed(1)}" stroke="${STROKE_CL}" stroke-width="0.7" stroke-dasharray="${DASH_CL}"/>`);

  // Marcas de compasso 0/90/180/270°
  const marcas: { graus: number; dx: number; dy: number; anchor: string }[] = [
    { graus: 0, dx: 0, dy: -4, anchor: 'middle' },
    { graus: 90, dx: 5, dy: 3, anchor: 'start' },
    { graus: 180, dx: 0, dy: 11, anchor: 'middle' },
    { graus: 270, dx: -5, dy: 3, anchor: 'end' },
  ];
  for (const marca of marcas) {
    const rad = ((marca.graus - 90) * Math.PI) / 180;
    const px = cx + (r + 12) * Math.cos(rad);
    const py = cy + (r + 12) * Math.sin(rad);
    parts.push(`<text x="${clamp(px + marca.dx, 12, VB_W - 12).toFixed(1)}" y="${clamp(py + marca.dy, 12, VB_H - 12).toFixed(1)}" font-size="7.5" text-anchor="${marca.anchor}" fill="#555">${marca.graus}°</text>`);
  }
  ...
  cota(parts, dx1, dy1, dx2, dy2, `Ø${fmt(d.D, 0)}`, VB_W);
  ...
  callout(parts, xAlvo, yAlvo, clamp(cx + r + 26, 12, VB_W - 40), clamp(cy - r - 14, 12, VB_H - 12), `t=${fmt(d.tCasco, 1)}`);
  ...
  for (const b of bocaisCasco) {
    ...
    const wPx = larguraBocalPx(b, scale);
    const tip = bocalFlangeado(parts, xBase, yBase, rad, 18, wPx);
    const bx = clamp(tip.x + 11 * Math.cos(rad), 12, VB_W - 12);
    const by = clamp(tip.y + 11 * Math.sin(rad), 12, VB_H - 12);
    idBalao(parts, bx, by, esc(b.id));

    if (a > 0.5 && a < 359.5) {
      const rArco = clamp(r + 24 + idxArco * 13, r + 12, Math.min(cx, cy) - 12);
      cotaAngular(parts, cx, cy, rArco, 0, a, `${fmt(a, 0)}°`, VB_W);
      idxArco++;
    }
  }
  ...
  for (const b of bocaisTampo) {
    ...
    const lado = clamp(larguraBocalPx(b, scale) * 1.6, 10, 26);
    parts.push(`<rect x="${(xB - lado / 2).toFixed(1)}" y="${(yB - lado / 2).toFixed(1)}" width="${lado.toFixed(1)}" height="${lado.toFixed(1)}" fill="none" stroke="${STROKE}" stroke-width="0.9" stroke-dasharray="${DASH_OCULTO}"/>`);
    parts.push(`<circle cx="${xB.toFixed(1)}" cy="${yB.toFixed(1)}" r="${(lado * 0.28).toFixed(1)}" fill="none" stroke="${STROKE}" stroke-width="0.7" stroke-dasharray="${DASH_OCULTO}"/>`);
    const bx = clamp(xB + lado / 2 + 12, 12, VB_W - 12);
    const by = clamp(yB - lado / 2 - 10 - idxTampo * 4, 12, VB_H - 12);
    parts.push(`<line x1="${xB.toFixed(1)}" y1="${yB.toFixed(1)}" x2="${(bx - 8).toFixed(1)}" y2="${by.toFixed(1)}" stroke="${STROKE}" stroke-width="0.6"/>`);
    idBalao(parts, bx, by, esc(b.id));
    idxTampo++;
  }

  // Circunferência anotada embaixo
  parts.push(`<text x="${cx.toFixed(1)}" y="${(VB_H - 8).toFixed(1)}" font-size="9" font-weight="700" text-anchor="middle" fill="#111">Circunf.: ${fmt(d.circunf, 1)} mm</text>`);
...
function svgDetalheTampo(tampo: TampoModelo, d: DadosBase): string {
  ...
  const AREA_W = VB_W - 2 * MARGIN - 76; // reserva p/ cotas h (dir) e t (esq)
  const AREA_H = VB_H - 150; // topo: título; baixo: casco + cota Ø
  ...
  const yBase = clamp(VB_H - 88, MARGIN, VB_H - MARGIN);
  ...
  parts.push(`<path d="${perfil.externo}" fill="${FILL_BRANCO}" stroke="${STROKE}" stroke-width="1.2"/>`);
  if (perfil.interno) {
    parts.push(`<path d="${perfil.interno}" fill="none" stroke="${STROKE}" stroke-width="0.9"/>`);
  }
  ...
  for (const lado of [-1, 1]) {
    ...
    parts.push(`<line x1="${xExt.toFixed(1)}" y1="${yBase.toFixed(1)}" x2="${xExt.toFixed(1)}" y2="${clamp(yBase + hCasco, MARGIN, VB_H - MARGIN).toFixed(1)}" stroke="${STROKE}" stroke-width="1.2"/>`);
    parts.push(`<line x1="${xInt.toFixed(1)}" y1="${yBase.toFixed(1)}" x2="${xInt.toFixed(1)}" y2="${clamp(yBase + hCasco, MARGIN, VB_H - MARGIN).toFixed(1)}" stroke="${STROKE}" stroke-width="0.9"/>`);
  }
  costura(parts, clamp(xEsq - 8, MARGIN, VB_W - MARGIN), yBase, clamp(xDir + 8, MARGIN, VB_W - MARGIN), yBase);

  // Título
  parts.push(`<text x="${xCentro.toFixed(1)}" y="16" font-size="10" font-weight="800" text-anchor="middle" fill="#111">${tipoTampoPorExtenso(tampo.tipo)}</text>`);

  // Cotas: Ø (abaixo do casco), profundidade h (direita), espessura t (callout na parede)
  const yCotaD = clamp(yBase + hCasco + 16, MARGIN, VB_H - MARGIN);
  cota(parts, xEsq, yCotaD, xDir, yCotaD, `Ø${fmt(d.D, 0)}`, VB_W);

  const xCotaProf = clamp(xDir + 22, MARGIN, VB_W - MARGIN);
  cota(parts, xCotaProf, yPonta, xCotaProf, yBase, `h=${fmt(dims.profundidade, 1)}`, VB_W);

  callout(
    parts,
    clamp(xEsq + tPx / 2, MARGIN, VB_W - MARGIN),
    clamp(yBase + hCasco / 2, MARGIN, VB_H - MARGIN),
    clamp(xEsq - 26, MARGIN + 4, VB_W - MARGIN),
    clamp(yBase + hCasco + 2, MARGIN, VB_H - MARGIN),
    `t=${fmt(t, 1)}`,
  );
  ...
  if (tampo.tipo === 'toriesferico') {
    callout(parts, clamp(xCentro + RextPx * 0.35, MARGIN, VB_W - MARGIN), clamp(yPonta + (yBase - yPonta) * 0.12, MARGIN, VB_H - MARGIN), clamp(xCentro + RextPx * 0.55, MARGIN, VB_W - MARGIN - 44), clamp(yPonta - 14, MARGIN + 8, VB_H - MARGIN), `Rc=${fmt(dims.raioCoroa, 0)}`);
    const xCanto = xCentro - RextPx * 0.93;
    const yCanto = yBase - (yBase - yPonta) * 0.32;
    callout(parts, clamp(xCanto, MARGIN, VB_W - MARGIN), clamp(yCanto, MARGIN, VB_H - MARGIN), clamp(xEsq - 12, MARGIN + 4, VB_W - MARGIN), clamp(yCanto - 22, MARGIN + 8, VB_H - MARGIN), `rc=${fmt(dims.raioCanto, 0)}`);
  }
